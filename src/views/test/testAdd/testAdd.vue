  ...
  <el-scrollbar height="97vh" class="create-container">
    ...
    >
      ...
      <div class="content">
        ...
        <div class="msg set">
          <el-form-item label="通过分数：" prop="pastscores">
            <el-input-number
              style="width: 90px"
              v-model="ruleForm.pastscores"
              :min="1"
              :max="120"
              size="small"
          ...
          <el-form-item label="开放时间：">
            <div class="date">
              <el-date-picker
                ...
                @change="getDate"
              />
              <span class="notfill">不填代表永久</span>
            </div>
          </el-form-item>
          <el-form-item label="答案解析：">
            <el-radio-group v-model="ruleForm.answershow" class="ml-4">
          ...
          <el-form-item label="防作弊：">
            ...
            <el-checkbox
              :true-label="1"
              :false-label="0"
              v-model="ruleForm.aorder"
              :label="1"
              size="large"
              >选项顺序打扰(单选题，多选题，判断题)</el-checkbox
            >
          </el-form-item>
        </div>
      </div>
      ...
      <div class="content">
        ...
        <div class="msg">
          <el-form-item label="考生范围：">
            <div class="choice">
              <el-button @click="handelStudent">
                <span>+选择</span>
              </el-button>
              <span class="counts">{{ ruleForm.students.length }}</span>
            </div>
          </el-form-item>
        </div>
      </div>
      ...
      <div class="content">
        ...
        <div class="msg">
          <el-form-item label="阅卷老师：">
            <div class="choice">
              <el-button @click="handlePaper">
                <span>+选择</span>
              </el-button>
              <span class="counts">{{ ruleForm.markteachers.length }}</span>
            </div>
          </el-form-item>
        </div>
      </div>
    ...
    />
    <!-- 批量导入试题 -->
    <batchExportDiog @onBatch="handleBatch" />
    <!-- 选择已有试卷 -->
    <testpaperDialog
...
const rules = reactive<FormRules>({
  ...
  info: [{ required: true, message: '请填写考试说明', trigger: 'blur' }],
  pastscores: [
...
const getDatabaseList = async () => {
  const res: any = await Test.reqDatabaseList();
...
const onAddTest = (state: number) => {
  ruleFormRef.value.validate(async (flag: boolean) => {
    if (flag) {
      ...
      if (ruleForm.pastscores > ruleForm.scores)
        return ElMessage.error('试题总分数不可低于通过分数');
      if (!ruleForm.limits.length) return ElMessage.error('请添加可见老师');
      if (!ruleForm.students.length) return ElMessage.error('请添加可见学生');
      if (!ruleForm.markteachers.length)
        return ElMessage.error('请添加阅卷老师');
      const res: any = await Test.reqConfirmTest(ruleForm);
      if (res.errCode === 10000) {
