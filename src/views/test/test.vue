<template>
  ...
  <div class="search">
    ...
    <div class="ismy">
      <el-checkbox v-model="ismy" @change="stateChange" label="我创建的" size="large" />
    </div>
    <div class="openTime">
      开放时间 <el-radio-group v-model="tableData.listConfig.opentime" class="ml-4">
        <el-radio :label="1" size="large">永久开放</el-radio>
        <el-radio :label="0" size="large">部分时段</el-radio>
      </el-radio-group>
    </div>
    <div class="timer">
      <el-date-picker v-model="timer" type="monthrange" unlink-panels range-separator="至" start-placeholder="开始日期"
        end-placeholder="结束日期" size="default" :shortcuts="shortcuts" @change="timesChange" />
    </div>
    状态 <el-select v-model="tableData.listConfig.state" class="m-2" placeholder="请选择"
      style="width:140px;margin-left:10px;">
      <el-option v-for="item in stateArr" :key="item" :label="item" :value="item" />
    </el-select>
  ...
  <el-table ref="multipleTableRef" :data="tableData.tableArr" style="width: 100%"
    ...
    <el-table-column property="pastscores" label="通过分数" show-overflow-tooltip />
    <el-table-column property="studentcounts" label="考试人数" show-overflow-tooltip />
    <el-table-column property="passCounts" label="通过人数" show-overflow-tooltip />
    <el-table-column label="开放时间" width="120">
      <template #default="scope">
        {{ scope.row.begintime & scope.row.endtime ? scope.row.begintime + '至' + scope.row.endtime : '不限' }}</template>
    </el-table-column>
    <el-table-column property="admin" label="创建人" show-overflow-tooltip />
    <el-table-column label="更新时间">
      <template #default="scope">{{ moment(scope.row.addtime).format('YYYY-MM-DD HH:mm') }}</template>
    </el-table-column>
...
// 日期选择器change事件
const timesChange = () => {
  tableData.listConfig.begindate = moment(timer.value[0]).format('YYYY-MM-DD HH:mm:ss')
  tableData.listConfig.enddate = moment(timer.value[1]).format('YYYY-MM-DD HH:mm:ss')
}
// 状态改变触发
const ismy = ref(false)
const stateChange = () => {
  ismy.value ? tableData.listConfig.ismy = 1 : tableData.listConfig.ismy = 0
  // 如果选择了我创建的 则创建人清空
